...
if (isset($_POST['cetak'])) {
    ...
    $cekid_jenis_surat = isset($id_jenis_surat);
    if ($tgl1 == $cektgl1 && $tgl2 == $cektgl2 && $id_jenis_surat == null) {
        ...
        $label = 'LAPORAN SURAT MASUK <br> Tanggal Terima Surat : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
    } else if ($tgl1 == null && $tgl2 == null && $id_jenis_surat == $cekid_jenis_surat) {
        $sql = mysqli_query($con, "SELECT * FROM surat_masuk a JOIN jenis_surat b ON a.id_jenis_surat = b.id_jenis_surat WHERE a.id_jenis_surat = '$id_jenis_surat' ORDER BY tgl_terima DESC");
        $dt = $con->query("SELECT * FROM jenis_surat WHERE id_jenis_surat = '$id_jenis_surat'")->fetch_array();
        $label = 'LAPORAN SURAT MASUK <br> Jenis Surat : ' . $dt['nm_jenis_surat'];
    } else if ($tgl1 == $cektgl1 && $tgl2 == $cektgl2 && $id_jenis_surat == $cekid_jenis_surat) {
        $sql = mysqli_query($con, "SELECT * FROM surat_masuk a JOIN jenis_surat b ON a.id_jenis_surat = b.id_jenis_surat WHERE a.tgl_terima BETWEEN '$tgl1' AND '$tgl2' AND a.id_jenis_surat = '$id_jenis_surat' ORDER BY tgl_terima ASC");
        $dt = $con->query("SELECT * FROM jenis_surat WHERE id_jenis_surat = '$id_jenis_surat'")->fetch_array();
        $label = 'LAPORAN SURAT MASUK <br> Tanggal Terima Surat : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2) . '<br> Jenis Surat : ' . $dt['nm_jenis_surat'];
    } else {
